...
exports.addQuestion = async(req,res,next)=>{
    ...
    //verifying quizId Named Collection is present or not.
        ...
        let checkQuizColl = await db.collection(quizColl).findOne({questionId:questionId});
        if(checkQuizColl){return next(new AppError("This Question is Already Added",401));}
...
exports.registerStudent = async(req,res,next)=>{
   ...
   if(!saveIt){return next(new AppError("Some Error Occured",403));;}
   else{
       res.send({
            ...
            credentials:`Login = `+saveIt.studentEnroll+`,Password(8 Character) = `+generate.key});
       }
};